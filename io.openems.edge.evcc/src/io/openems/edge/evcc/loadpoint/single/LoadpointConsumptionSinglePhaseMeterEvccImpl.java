...
public class LoadpointConsumptionSinglePhaseMeterEvccImpl extends AbstractOpenemsComponent implements
	...
	private void processHttpResult(HttpResponse<JsonElement> result, HttpError error) {
		if (error != null) {
			this.logDebug(log, error.getMessage());
			return;
		}

		try {
			this.logDebug(log, "processHttpResult");
			...
			if (lp.has("chargeVoltages") && lp.get("chargeVoltages").isJsonArray()) {
				...
				this.logDebug(log, "chargeVoltages not provided or null – defaulting voltages");
			...
			if (lp.has("chargeCurrents") && lp.get("chargeCurrents").isJsonArray()) {
				...
				this.logDebug(log, "chargeCurrents not provided or null – estimating phase current mapping.");
			...
			log.warn("Failed to parse evcc loadpoint data: {}", e.getMessage());
		}
	}
